...
//        }
    ...
    fun checkTripleByPlayer(player: UUID, bvid: String): TripleCheckResult {
        val bound = BoundAccountRepository.findByPlayerUuid(player.toString())
            ?: return TripleCheckResult(
                code = TripleCheckCode.PLAYER_NOT_BOUND,
                message = "你还没有绑定 B 站账号。"
            )
        ...
        return internalCheckTripleWithCredentialNoSQL(entity, bvid)
    }
    /**
     * 使用给定凭证实体执行三连检测的公共逻辑。
     */
    ...
    /**
     * 使用给定凭证实体执行三连检测的公共逻辑（去除数据库更新）。
     */
    private fun internalCheckTripleWithCredentialNoSQL(
        entity: Credential,
        bvid: String,
    ): TripleCheckResult {
        if (entity.status == 0) {
        ...
        } catch (ex: BilibiliHttpClient.BilibiliRiskControlException){
            warning("B 站接口触发 412 风控，接口将在冷却后重试")
            TripleCheckResult(
                code = TripleCheckCode.TRIPLE_CHECK_FAILED,
